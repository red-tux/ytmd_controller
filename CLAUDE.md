...
- `actions/<Name>/<Name>.py` — one class per action, each `class X(YTMDKeyAction)` or `class X(YTMDDialAction)` (defined in `ytmd_action_base.py`). Never subclass the deprecated `ActionBase`.
- `actions/common/ytmd_action_base.py` — `YTMDActionBase`: the shared plumbing (no `__init__`, so it doesn't disturb the MRO), plus `YTMDKeyAction(YTMDActionBase, KeyAction)` / `YTMDDialAction(YTMDActionBase, DialAction)` for concrete actions to subclass. Provides state subscribe/unsubscribe, the thumbnail request wrapper, all the rendering helpers (label rows, progress bar, icon/colour asset compositing, pause overlay, `get_display_size()`), and the render-arbitration layer: actions render through `push_media()` / `push_*_label()`, which no-op while the "Check YTMD Settings" auth-error overlay owns the input (`_showing_auth_error`), so a late setting-change/thumbnail/timer repaint can't paint over it. `_render_auth_error()` / `_clear_auth_error()` bypass that via raw `self.ui()` — they *are* the arbiter, fed by `state_store.set_auth_ok()` (startup `check_auth()`, the settings dialog probe, or a live 401 in `send_command()`).
...
- **GTK/Adw calls must be on the main thread** (parent-repo hard constraint). `YTMDActionBase.ui(fn, *args)` = `GLib.idle_add(lambda: fn(*args))` is the marshalling helper; `set_media`/`set_label` calls from any off-thread context go through it.
...
- `YTMDActionBase.on_ready()` subscribes to `state_store` **idempotently** (`getattr(self, "_state_token", None) is None` guard) — an unconditional subscribe would leak a subscription per page revisit. `on_disconnect()` unsubscribes and nulls the token, so a genuine teardown/re-ready still re-subscribes.
...
Actions that expose multiple functions on one input pass `default_events=False` to the base and register explicit `EventAssigner`s (`add_event_assigner(EventAssigner(id=..., ui_label=..., default_events=[...], callback=...))`), so a user can bind e.g. press→Next, hold→Previous on a single key, and rebind any of it in the UI. `YTMDActionBase` overrides the unused `on_key_*` / `on_dial_*` no-ops with a `data=None` signature, because `EventAssigner.call(*args)` always forwards the hardware callback's data (even `None`) and the base classes' zero-arg defaults would crash.
...
All hardware output goes through `push_media()` / `push_top_label()` / `push_center_label()` / `push_bottom_label()` on the base — never `self.ui(self.set_media, …)` or `set_*_label` directly. That's the single choke point the auth-error overlay uses to stay on top: while `_showing_auth_error` is set, the `push_*` calls no-op, so a repaint triggered by a GenerativeUI `on_change` (they *do* fire on initial `load_initial_ui`), a thumbnail callback, or a `DialControl` timer can't clobber the "Check YTMD Settings" screen. On recovery the base re-enters `on_ready()` for a clean repaint.
